...
public class HTTPHC4Impl extends HTTPHCAbstractImpl{
	...
	private static final String CONTEXT_METRICS = "jmeter_metrics"; // TODO hack for metrics related to HTTPCLIENT-1081,
																	// to be removed later
	 ...
	 * localContext.getAttribute(ExecutionContext.HTTP_CONNECTION)).getMetrics();
	 ...
	 */
	private static final ThreadLocal<Map<HttpClientKey, HttpClient>> HTTPCLIENTS_CACHE_PER_THREAD_AND_HTTPCLIENTKEY = new ThreadLocal<Map<HttpClientKey, HttpClient>>(){
		@Override
	...
	static{
		...
		if(CPS_HTTPS > 0){
			...
			try{
				https = new Scheme(HTTPConstants.PROTOCOL_HTTPS, HTTPConstants.DEFAULT_HTTPS_PORT,
						new SlowHC4SSLSocketFactory(CPS_HTTPS));
			...
			try{
				https = new Scheme(HTTPConstants.PROTOCOL_HTTPS, HTTPConstants.DEFAULT_HTTPS_PORT,
						new HC4TrustAllSSLSocketFactory());
			}catch(GeneralSecurityException e){
	...
	protected HTTPSampleResult sample(URL url, String method, boolean areFollowingRedirect, int frameDepth){
		...
		try{
			...
			final InetAddress localAddr = (InetAddress) httpRequest.getParams().getParameter(
			...
			if(entity != null){
				...
				if(SAVE_RESPONSE_DATA){
					...
					readResponse(res, instream, (int) entity.getContentLength());
				}
			}
			...
			res.setRequestHeaders(getConnectionHeaders((HttpRequest) localContext
	 ...
	 * {@link #sendEntityData(HttpEntityEnclosingRequestBase)} if method is <code>PUT</code> or <code>PATCH</code>
	 * <p>
	...
	protected void handleMethod(String method, HTTPSampleResult result, HttpRequestBase httpRequest,
		...
		if(method.equals(HTTPConstants.POST)){
			...
			result.setQueryString(postBody);
				...
				|| HttpWebdav.isWebdavMethod(method) || method.equals(HTTPConstants.DELETE)){
	...
	private static final class HttpClientKey{
		...
		public HttpClientKey(URL url, boolean hasProxy, String proxyHost, int proxyPort, String proxyUser,
				String proxyPass){
			// N.B. need to separate protocol from authority otherwise http://server would match https://erver (<= sic,
			// not typo error)
	...
	private HttpClient setupClient(URL url, SampleResult res){
		...
		HttpClientKey key = new HttpClientKey(url, (useStaticProxy || useDynamicProxy), useDynamicProxy ? proxyHost
				: PROXY_HOST, useDynamicProxy ? proxyPort : PROXY_PORT, useDynamicProxy ? getProxyUser() : PROXY_USER,
				...
				while(i.hasNext()){
					...
					if(!HTTPConstants.HEADER_CONTENT_LENGTH.equalsIgnoreCase(n)){
						...
						if(HTTPConstants.HEADER_HOST.equalsIgnoreCase(n)){
							...
							v = v.replaceFirst(":\\d+$", ""); // remove any port specification // $NON-NLS-1$
																// $NON-NLS-2$
							if(port != -1){
	...
	protected String sendPostData(HttpPost post) throws IOException{
		...
		if(getUseMultipartForPost()){
			...
			if(multiPart.isRepeatable()){
				...
				postedBody.append(new String(bos.toByteArray(), contentEncoding == null ? "US-ASCII" // $NON-NLS-1$ this
																										// is the
																										// default used
																										// by HttpClient
						: contentEncoding));
			...
			if(!hasArguments() && getSendFileAsPostBody()){
				...
				FileEntity fileRequestEntity = new FileEntity(new File(file.getPath()), (ContentType) null);// TODO is
																											// null
																											// correct?
	 ...
	 * <p>
	 * If there is a file entry with a non-empty MIME type we use that to set the request Content-Type header, otherwise
	 * we default to whatever header is present from a Header Manager.
	 * <p>
	 * If the content charset {@link #getContentEncoding()} is null or empty we use the HC4 default provided by
	 * {@link HTTP#DEF_CONTENT_CHARSET} which is ISO-8859-1.
	 * 
	...
	protected String sendEntityData(HttpEntityEnclosingRequestBase entity) throws IOException{
		...
		if(!hasArguments() && getSendFileAsPostBody()){
			...
			// If getSendFileAsPostBody returned true, it's sure that file is not null
			FileEntity fileRequestEntity = new FileEntity(new File(files[0].getPath())); // no need for content-type
																							// here
