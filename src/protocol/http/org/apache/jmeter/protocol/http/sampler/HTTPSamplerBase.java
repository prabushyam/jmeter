 ...
 */
public abstract class HTTPSamplerBase extends AbstractSampler implements TestStateListener, TestIterationListener,
		ThreadListener, HTTPConstantsInterface{
	...
	private static final long AWAIT_TERMINATION_TIMEOUT = JMeterUtils.getPropDefault(
			"httpsampler.await_termination_timeout", 60); // $NON-NLS-1$ // default value: 60 secs

	private static final boolean IGNORE_FAILED_EMBEDDED_RESOURCES = JMeterUtils.getPropDefault(
			"httpsampler.ignore_failed_embedded_resources", false); // $NON-NLS-1$ // default value: false

	public static final int CONCURRENT_POOL_SIZE = 4; // Default concurrent pool size for download embedded resources

	private byte[] readBuffer = new byte[8192]; // 8kB is the (max) size to have the latency ('the first packet')
	...
	private static final String[] METHODS = {
			...
			HTTPConstants.REPORT, HTTPConstants.MKCALENDAR };
	...
	private static final boolean REMOVESLASHDOTDOT = JMeterUtils.getPropDefault(
			"httpsampler.redirect.removeslashdotdot", true);

	private static final boolean SAVE_RESPONSE_DATA = JMeterUtils.getPropDefault(
	 ...
	 * does not start with http[s]://) it also calls {@link #parseArguments(String, String)} to extract and store any
	 * query arguments.
	 * 
	 * @param path
	 *            The new Path value
	...
	public static int getDefaultPort(String protocol, int port){
		if(port == URL_UNSPECIFIED_PORT){
			return protocol.equalsIgnoreCase(HTTPConstants.PROTOCOL_HTTP) ? HTTPConstants.DEFAULT_HTTP_PORT : protocol
					.equalsIgnoreCase(HTTPConstants.PROTOCOL_HTTPS) ? HTTPConstants.DEFAULT_HTTPS_PORT : port;
		}
		return port;
	}
	...
	/**
	 * Samples the URL passed in and stores the result in <code>HTTPSampleResult</code>, following redirects and
	 * downloading
	 * page resources as appropriate.
	 * <p>
	 * When getting a redirect target, redirects are not followed and resources are not downloaded. The caller will take
	 * care of this.
	 * 
	 * @param u
	 *            URL to sample
					...
					if(url == null){
						...
						if(isConcurrentDwn()){
							...
							setParentSampleSuccess(res, res.isSuccessful()
									&& (binRes != null ? binRes.isSuccessful() : true));
						}

					}
				}catch(ClassCastException e){ // TODO can this happen?
					res.addSubResult(errorResult(new Exception(binURL + " is not a correct URI"), new HTTPSampleResult(
							res)));
					setParentSampleSuccess(res, false);
					continue;
				}
	 ...
	 * <p>
	 * The returned result will contain one subsample for each request issued, including the original one that was
	 * passed in. It will be an HTTPSampleResult that should mostly look as if the final destination of the redirect
	 * chain had been obtained in a single shot.
	 * 
	 * @param res
	...
	public byte[] readResponse(SampleResult sampleResult, InputStream in, int length) throws IOException{
		try{
			...
			if(SAVE_RESPONSE_DATA){
				w = new ByteArrayOutputStream(bufferSize);
			}
			int bytesRead = 0;
			int totalBytes = 0;
			boolean first = true;
	 ...
	 * <p>
	 * In order to maintain backwards compatibility of test plans, the 3 original properties were retained; additional
	 * file entries are stored in an HTTPFileArgs class. The HTTPFileArgs class was only present if there is more than 1
	 * file; this means that such test plans are backward compatible.
	 * <p>
	 * Versions after 2.3.4 dispense with the original set of 3 properties. Test plans that use them are converted to
	 * use a single HTTPFileArgs list.
	 * 
